...
if [ -z "$LAYER_SHELL_LIB" ]; then
    ...
    if command -v ldconfig >/dev/null 2>&1; then
        LAYER_SHELL_LIB=$(ldconfig -p 2>/dev/null | grep 'libgtk4-layer-shell.so.0 ' | head -1 | sed 's/.* => //')
    fi
fi
...
# Generate the notify-send shim drop-in with the user's current PATH prepended
# This preserves the user's existing PATH instead of replacing it
SHIM_PATH="$HOME/.local/lib/voxtype-shims"
# Escape % to %% for systemd specifier expansion, strip any newlines
SAFE_PATH=$(printf '%s' "${SHIM_PATH}:${PATH}" | tr -d '\n' | sed 's/%/%%/g')
